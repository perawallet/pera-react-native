...
| `pera/no-chain-identity-in-shared-packages` | The chain-independent packages never branch on `.chainId`/`.family` (`switch`, `===`, `!==`) or spell a `ChainId` as a string literal; permanent exceptions are listed, with reasons, in the rule. Ships as `warn`: its spec pins the existing offenders. |
...
| `pera/spec-file-suffix`                     | A test file ends in `.spec.*`, never `.test.*`: the vitest configs collect only `*.spec.*`.                                                                                                                                                               |
