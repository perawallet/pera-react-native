...
| `pera/no-wc-imports-in-connections-module` | The connections module and the deeplink handlers reach WalletConnect through the registry, never `@perawallet/wallet-core-walletconnect`.                                                                   |
| `pera/no-chain-package-imports`            | Only a composition root, test code or the chain package itself imports a chain package; no chain imports another. Roots and whole-move shims, with reasons: `lanekeep/shared/chain-package-allowlist.ts`.   |
