...
| `pera/no-chain-package-imports`            | Only a composition root or the chain package itself imports a chain package; everything else goes through `getProvider().chains`. The allowlist and each entry's reason are in the rule.                    |
