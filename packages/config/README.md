...
2. `generatedEnv` from `src/generated-env.ts`, written by `tools/dev/generate-config.sh` from the
...
`tools/dev/generate-config.sh` sources `.env` at the repo root, then an optional overlay file named by
...
3. An `append_config` line in `tools/dev/generate-config.sh`, or the variable never reaches
   `generated-env.ts`.
