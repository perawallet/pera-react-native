...
describe('backupWebSocketUrl', () => {
    it('swaps https→wss, keeps backupId raw in the path, and url-encodes query params', () => {
        ...
        expect(url.startsWith('wss://staging.backup.perawallet.app/api/v3/backup/did:pera:ABC?')).toBe(true)
    ...
    it('swaps http→ws for a local base url', () => {
        const url = backupWebSocketUrl({
            baseUrl: 'http://localhost:3000', backupId: 'b', deviceId: 'd', timestamp: 't', signature: 's',
        })
        expect(url.startsWith('ws://localhost:3000/api/v3/backup/b?')).toBe(true)
    })
})
