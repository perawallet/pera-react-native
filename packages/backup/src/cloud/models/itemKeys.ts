...
/** `hashItemAddress` emits lowercase hex, so the casing is part of the match:
 *  anything else in the segment was not written by this device's hasher. */
const HASHED_SEGMENT = /^[0-9a-f]{64}$/

/** An item written before keys were hashed, whose segment is a plaintext
 *  address. Such a backup is re-created rather than migrated: a re-key would
 *  leave the old addresses in the server's changelog anyway. */
