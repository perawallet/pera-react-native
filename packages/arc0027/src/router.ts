...
// Platform-agnostic ARC-0027 dapp-connect core: authenticates against an
// already-extracted origin string, consults the permission store, and
// answers discover/disable/already-approved-enable directly. A fresh enable
// opens the approval window via an injectable Arc0027ApprovalOpener and
// parks the request until the window resolves it.
//
// This class has no transport of its own (no chrome.*, no message-listener
// wiring) — a transport binding (e.g. extensions/platform-chrome's
// ChromeDappRouter, or a future mobile WebRTC/CBOR transport) is responsible
// for extracting a trusted `origin` from its own channel and handing this
// class an already-parsed request via `handle()`.
import {
    ARC0027_ERROR_CODES,
...
export class DappRequestRouter {
    ...
    // Single public entry point: a transport hands this an unvalidated
    // `request` payload plus a trust-boundary-checked `origin` (never a
    // page-asserted field — the caller must have already extracted it from
    // its own channel, e.g. chrome.runtime.MessageSender.origin) and an
    // optional `faviconUrl`. Validates the request shape itself (folding in
    // what used to be the transport's isArc0027Request + malformed-request
    // handling, since that validation is protocol-shape logic, not
    // transport logic) before dispatching.
    async handle(
        request: unknown,
        origin: string,
        faviconUrl?: string,
        ...
        if (!isArc0027Request(request)) {
            return malformedRequestError(request)
        }
