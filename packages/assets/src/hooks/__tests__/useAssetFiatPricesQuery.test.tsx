...
describe('useAssetFiatPricesQuery', () => {
    ...
    it('fetches and converts asset prices correctly', async () => {
        ...
        expect(result.current.data.get(ALGO_ASSET_ID)?.fiatPrice).toEqual(new Decimal(3.0)) // 1.5 * 2
        expect(result.current.data.get('123')?.fiatPrice).toEqual(new Decimal(4.0)) // 2.0 * 2
    })
    ...
    it('filters out assets with null usd_value', async () => {
        ...
        expect(result.current.data.get('456')?.fiatPrice).toEqual(new Decimal(0))
        expect(result.current.data.get(ALGO_ASSET_ID)?.fiatPrice).toEqual(new Decimal(3.0))
    })
