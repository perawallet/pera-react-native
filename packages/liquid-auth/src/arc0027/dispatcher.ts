...
export const createArc0027Dispatcher = (
    ...
    return async (raw: string) => {
        ...
        try {
            ...
            // Diagnostic: a non-ARC-0027 frame the dApp sent that we can't
            // parse (wrong envelope/reference). Log a short, payload-free
            // prefix so we can see the shape without dumping txn data.
            logger.info('[liquid-auth] dispatcher: unparseable frame', {
                length: raw.length,
                prefix: raw.slice(0, 80),
            })
            return null
        }
        ...
        try {
            ...
            // Anomaly worth keeping: a handler rejected (user declined, bad
            // input, unsupported method). Routine success is not logged.
            logger.info('[liquid-auth] dispatcher: handler error', {
                method,
                code: arcError.code,
