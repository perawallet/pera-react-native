...
describe('services/accounts/useHDWallet', () => {
    ...
    test('generateMasterKey accepts custom mnemonic', async () => {
        vi.resetModules()
        const customMnemonic = 'custom test mnemonic phrase'

        const { useHDWallet } = await import('../useHDWallet')
        const { result } = renderHook(() => useHDWallet())
        await result.current.generateMasterKey(customMnemonic)
    ...
    test('entropyToMnemonic throws when bip39.entropyToMnemonic fails', async () => {
        vi.resetModules()
        bip39Spies.entropyToMnemonic.mockImplementationOnce(() => {
