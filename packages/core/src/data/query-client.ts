...
const createFetchClient = (clients: Map<string, BackendInstances>) => {
    ...
    return async <TData, _TError = unknown, TVariables = unknown>(
        ...
        const backends = clients.get(network)

        if (!backends) {
            throw new Error('Could not get backends for ' + network)
        }

        const client = backends[requestConfig.backend]

        if (!client) {
            throw new Error('Could not get KY client for ' + requestConfig.backend)
        }
...
//TODO rename after we remove kubb autogenerated version
export const updateManualBackendHeaders = (headers: Map<string, string>) => {
