...
describe('migrateStashed', () => {
    it('writes nothing and reports not stashed when the blob is null', () => {
        const result = migrateStashed({
            walletConnectHistoryBlob: null,
        })

        expect(result).toEqual({ walletConnectHistoryBlobStashed: false })
        expect(settingsMock.setPreference).not.toHaveBeenCalled()
    })

    it('never stashes passkeys (native import is the source of truth)', () => {
        migrateStashed({ walletConnectHistoryBlob: null })

        expect(settingsMock.setPreference).not.toHaveBeenCalledWith(
            'legacy.passkeys',
            expect.anything(),
        )
    })
    ...
    it('stashes the walletConnect history blob when provided', () => {
        ...
        expect(result.walletConnectHistoryBlobStashed).toBe(true)
    ...
    it('skips the blob preference when undefined', () => {
        ...
        expect(result.walletConnectHistoryBlobStashed).toBe(false)
        expect(settingsMock.setPreference).not.toHaveBeenCalled()
    })
})
