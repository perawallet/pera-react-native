...
describe('useWalletConnect', () => {
    ...
    describe('connectSession', () => {
        it('should initialize connector and bind events', async () => {
            ...
            const session = { session: { clientId: 'test-session', topic: 'abc', bridge: 'xyz', key: '123' } } as any
            ...
            const mockConnectorInstance = (WalletConnect as any).mock.results[0].value
            expect(mockConnectorInstance.on).toHaveBeenCalledWith('algo_signData', expect.any(Function))
            expect(mockConnectorInstance.on).toHaveBeenCalledWith('algo_signTxn', expect.any(Function))
            expect(mockConnectorInstance.on).toHaveBeenCalledWith('disconnect', expect.any(Function))
            expect(mockConnectorInstance.on).toHaveBeenCalledWith('session_request', expect.any(Function))
            expect(mockConnectorInstance.on).toHaveBeenCalledWith('error', expect.any(Function))
        })
        ...
        it('should handle session_request event', async () => {
            ...
            const mockConnectorInstance = (WalletConnect as any).mock.results[0].value
            const sessionRequestCallback = mockConnectorInstance.on.mock.calls.find((call: any) => call[0] === 'session_request')[1]

            const payload = {
                params: [{ peerMeta: { name: 'App' }, chainId: 4160, permissions: ['perm1'] }]
            }
        ...
        it('should handle disconnect event', async () => {
            ...
            const mockConnectorInstance = (WalletConnect as any).mock.results[0].value
            const disconnectCallback = mockConnectorInstance.on.mock.calls.find((call: any) => call[0] === 'disconnect')[1]
    ...
    describe('disconnectSession', () => {
        it('should kill session and remove from store', async () => {
            ...
            expect(mockConnectorInstance.killSession).toHaveBeenCalledWith({ message: 'User disconnected' })
            expect(mockSetSessions).toHaveBeenCalled()
    ...
    describe('approveSession', () => {
        it('should approve session and update store', async () => {
            ...
            act(() => {
                result.current.approveSession('mock-client-id', request, addresses)
            })
            ...
            const updatedSessions = mockSetSessions.mock.calls[mockSetSessions.mock.calls.length - 1][0]
    ...
    describe('deleteAllSessions', () => {
        it('should kill all sessions and clear store', async () => {
            ...
            const connector1 = { clientId: 'client1', killSession: vi.fn(), connected: true }
            const connector2 = { clientId: 'client2', killSession: vi.fn(), connected: true }
            ...
            await act(async () => {
                await result.current.connectSession({ session: { clientId: 'client1' } } as any)
                await result.current.connectSession({ session: { clientId: 'client2' } } as any)
            })
    ...
    describe('error handling and edge cases', () => {
        it('should handle error event', async () => {
            const { result } = renderHook(() => useWalletConnect())
            await act(async () => {
                await result.current.connectSession({ session: { clientId: 'test' } } as any)
            })
            const mockConnectorInstance = (WalletConnect as any).mock.results[0].value
            const errorCallback = mockConnectorInstance.on.mock.calls.find((call: any) => call[0] === 'error')[1]
