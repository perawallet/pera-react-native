...
export function fromSeed(seed: Buffer): Uint8Array {
    ...
    // chain root code
    // SHA256(0x01||k)
    const c: Buffer = createHash('sha256')
...
export async function deriveChildNodePrivate(
    ...
    const { z, childChainCode } =
        index < 0x80000000
            ? await derivedNonHardened(kL, cc, index)
...
export async function deriveChildNodePublic(
    ...
    const { crypto_scalarmult_ed25519_base_noclamp, crypto_core_ed25519_add } =
        await loadLibSodium()
    ...
    data[0] = 0x03
    const fullChildChainCode: Buffer = createHmac('sha512', cc)
        .update(data)
        .digest()
 ...
 */
async function derivedNonHardened(
    kl: Uint8Array,
    cc: Uint8Array,
    index: number,
): Promise<{ z: Uint8Array; childChainCode: Uint8Array }> {
    const data: Buffer = Buffer.allocUnsafe(1 + 32 + 4)
    data.writeUInt32LE(index, 1 + 32)
