...
describe('Contextual Derivation & Signing', () => {
    ...
    describe('\(JS Library) Reference Implementation alignment with known BIP32-Ed25519 JS LIB', () => {
        test("\(OK) BIP32-Ed25519 derive key m'/44'/283'/0'/0/0", async () => {
            ...
            derivedKey = otherLibBip32Ed25519.derivePrivate(
                derivedKey,
                harden(283),
            )
            derivedKey = otherLibBip32Ed25519.derivePrivate(
            ...
            const { crypto_scalarmult_ed25519_base_noclamp } =
                await loadLibSodium()
    ...
    describe('\(Derivations) Context', () => {
        ...
        describe('Signing Typed Data', () => {
            test('\(OK) Sign authentication challenge of 32 bytes, encoded base64', async () => {
                ...
                const isValid: boolean =
                    await cryptoService.verifyWithPublicKey(
                        signature,
                        encoded,
                        await cryptoService.keyGen(
                            rootKey,
                            KeyContexts.Address,
                            0,
                            0,
            ...
            test('\(OK) Sign Arbitrary Message against Schema, encoded base64', async () => {
                ...
                const encoded: Buffer = Buffer.from(
                    to_base64(JSON.stringify(message)),
                )
        ...
        describe('signing transactions', () => {
            test('\(OK) Sign Transaction', async () => {
                ...
                await expect(
                    nacl.sign.detached.verify(prefixEncodedTx, sig, key),
                ).toBe(true)
            })
        })
        ...
        describe('ECDH cases', () => {
            ...
            test('Libsodium example ECDH', async () => {
                ...
                // shared secret
                const aliceSecret: Uint8Array = crypto_scalarmult(
                    aliceXPvt,
                    bobXPub,
                )
                const bobSecret: Uint8Array = crypto_scalarmult(
                    bobXPvt,
                    aliceXPub,
                )
                await expect(aliceSecret).toEqual(bobSecret)

                const aliceSharedSecret: CryptoKX =
                    crypto_kx_client_session_keys(aliceXPub, aliceXPvt, bobXPub)
                ...
                await expect(aliceSharedSecret.sharedRx).toEqual(
                    bobSharedSecret.sharedTx,
                )
                await expect(bobSharedSecret.sharedTx).toEqual(
                    aliceSharedSecret.sharedRx,
                )
            })
        })
    })
    ...
    describe('\(deriveNodePrivate)', () => {
        test('\(FAIL) Should fail if during derivation scalar >= 2^255', async () => {
            ...
            await expect(
                deriveChildNodePrivate(derivationNode, 0, g),
            ).rejects.toThrow('zL * 8 is larger than 2^255, which is not safe')
        })
    })
