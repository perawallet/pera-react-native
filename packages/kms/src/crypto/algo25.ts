...
import { executeWithKey, getSeedFromMasterKey, saveKey } from '../utils'
import { encodeAddress } from '@algorandfoundation/algokit-utils'

export const generateAlgo25Key = async (
    mnemonic?: string,
): Promise<Uint8Array> => {
    if (mnemonic) {
...
export const algo25Sign = (seed: Uint8Array, data: Uint8Array): Uint8Array => {
...
export const createAlgo25Key = async (params: {
    id?: string
    mnemonic?: string
}) => {
...
export const withAlgo25Session = async <T>(
    key: KeyPair,
    domain: string,
