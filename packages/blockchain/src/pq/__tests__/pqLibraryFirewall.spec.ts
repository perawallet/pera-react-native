...
const isAllowlistedSeamPath = (root: string, path: string): boolean =>
    ALLOWLISTED_SEAM_DIRS.some(seamDir => path.startsWith(join(root, seamDir)))
...
const listSourceFilesRecursively = (root: string, dir: string): string[] =>
    readdirSync(dir, { withFileTypes: true }).flatMap(entry => {
        ...
        if (entry.isDirectory()) {
            ...
            if (isAllowlistedSeamPath(root, fullPath)) return []
