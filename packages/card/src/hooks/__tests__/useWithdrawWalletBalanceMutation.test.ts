...
vi.mock('../../api/wallet-balance', () => ({ withdrawWalletBalance }))
...
describe('useWithdrawWalletBalanceMutation', () => {
    ...
    it.each([CardWalletKind.Reward, CardWalletKind.Credit])(
        'withdraws from the %s wallet and invalidates only that wallet',
        async kind => {
            withdrawWalletBalance.mockResolvedValue({
                txHash: '0xabc',
                network: 'linea',
                isConfirmed: true,
            })
            ...
            expect(withdrawWalletBalance).toHaveBeenCalledWith({
                kind,
                amount: '10.5',
                network: 'mainnet',
            })
