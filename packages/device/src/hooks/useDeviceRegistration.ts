...
export const useDeviceRegistration = (addresses: string[]): void => {
    ...
    // ticks). Key the effect on the joined address set so it only refires
    // when membership actually changes — one store write must not mean one
    // device PUT.
    const addressesKey = useMemo(() => addresses.join('\n'), [addresses])
