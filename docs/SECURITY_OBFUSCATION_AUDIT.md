...
The audit gaps were remediated for the first release. All changes live in `apps/mobile`; the Android native project is generated by `expo prebuild`, so the Expo config and config-plugins are the source of truth (editing the generated `android/` files directly would be overwritten).

| #   | Change                                       | File(s)                                                                                                                        | Effect                                                         |
| --- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| 1   | Enable R8 minify + resource shrinking        | `app.config.builder.js` (`enableMinifyInReleaseBuilds`, `enableShrinkResourcesInReleaseBuilds`, baseline `extraProguardRules`) | Obfuscates native Java/Kotlin, strips unused code/resources    |
| 2   | Right-size duplicated / oversized images     | `assets/images/banners/{retail,staking,card}-bg*`, `assets/images/pera-card.png`                                               | Shipped image payload 5988 → 4184 KB (~1.8 MB)                 |
| 3   | Stop shipping the dev render-profiler        | `src/wdyr.ts` (`__DEV__ &&` guard)                                                                                             | `why-did-you-render` removed from prod bundle (verified 1 → 0) |
| 4   | Upload Android native symbols to Crashlytics | new `plugins/withAndroidNativeSymbolUpload.js` (+ unit test)                                                                   | Symbolicated Android native crashes (iOS dSYM parity)          |

Audited but intentionally **not** changed: `tweetnacl` (load-bearing crypto — Algo25 + multisig signing — kept); WebP conversion (declined; PNG right-sizing only); the JS layer's transitive heavies (all load-bearing — SVG, Ledger, Algorand SDK, UI kit).

**Verification (local):** `pnpm build` / `tsc --noEmit` ✅ · type-aware lint ✅ · i18n ✅ · secret scan ✅ · plugin unit tests ✅ (4/4) · `expo prebuild` applies the R8 flags and native-symbol config to the generated `build.gradle` ✅ · production bundle rebuilds with `why-did-you-render` absent ✅. **Not run locally (release smoke-test gate):** a signed release build — to confirm R8 keep-rules, the native-symbol upload running in CI, and banner/card visuals at the new resolutions.
...
### JS bundle composition (source-map analysis)

Aggregating the production source map by package (source bytes — a proxy for contribution; the shipped ratio shifts after minification + Hermes + tree-shaking):

| Package                 | ~Source            | Notes                                             |
| ----------------------- | ------------------ | ------------------------------------------------- |
| app (`apps/mobile`)     | 8.4 MB (2216 mods) | Own code + 165 SVGs compiled to components        |
| react-native            | 2.4 MB             | Framework                                         |
| zod                     | 1.1 MB             | Validation                                        |
| react-native-reanimated | 0.9 MB             | Animation                                         |
| algokit-utils           | 0.8 MB             | Algorand SDK (core)                               |
| lodash                  | 0.5 MB             | transitive ← `@rneui/base` → react-native-ratings |
| rxjs                    | 0.5 MB             | transitive ← `@ledgerhq/*` (Ledger transport)     |
| css-tree + mdn-data     | 0.6 MB             | transitive ← `react-native-svg` (SVG CSS)         |
| ajv                     | 0.2 MB             | transitive ← `algokit-utils` / keystore           |

**Conclusion:** there is no single removable multi-MB dependency. Every notable third-party heavy is transitive and load-bearing (SVG, Ledger, Algorand SDK, UI kit), so the realistic size levers remain R8 + resource-shrinking (native) and asset right-sizing — all already applied.

**Finding & fix — dev profiler shipped to production (✅ fixed):** `@welldone-software/why-did-you-render` (a dev-only render profiler, ~55 KB) was being bundled into release builds. Although correctly in `devDependencies`, `apps/mobile/src/wdyr.ts` guarded its `require()` with the **runtime** `config.profilingEnabled`, which Metro cannot dead-code-eliminate (only compile-time `__DEV__` branches are stripped). Re-guarded with `__DEV__ && config.profilingEnabled`; **verified** the module is now absent from the production bundle (occurrences 1 → 0).
...
| Audit crypto lib overlap                           | **Audited — no change**                       | `tweetnacl` is **not** redundant: it performs Algo25 key derivation (`packages/kms`) and multisig signature assembly (`packages/blockchain`), and is a direct dep in 4 packages. It is tiny (<55 KB) and replacing it is a high-risk crypto change with no size payoff. **Kept.** |
...
3. **[Low] Upload Android native symbols to Crashlytics** — **✅ Implemented** via the custom Expo config plugin `apps/mobile/plugins/withAndroidNativeSymbolUpload.js`, which patches the (prebuild-generated) release buildType with `ndk { debugSymbolLevel 'FULL' }` + `firebaseCrashlytics { nativeSymbolUploadEnabled true }` — the Android counterpart to the iOS dSYM upload. Diagnostics, not security/size: symbols are uploaded to Crashlytics / embedded in AAB metadata, not delivered to devices. **Validate in a CI release build.** Symbol _upload_ is separate from native-crash _capture_ — if the latter is also wanted on Android, confirm the `firebase-crashlytics-ndk` dependency is present (follow-up).
4. **[Low] Asset optimization** — **✅ Implemented** (`apps/mobile/assets`): right-sized the over-provisioned/duplicated banner PNGs and downscaled `pera-card.png` (see §5), cutting the shipped image-asset payload from **5988 KB → 4184 KB (~1.8 MB)** before AAPT crunch / R8 resource-shrink. JS Hermes bytecode is unchanged (~16.6 MB — image assets are not part of the JS bundle). Needs the release smoke test to confirm banners/card render correctly at the new resolutions.
