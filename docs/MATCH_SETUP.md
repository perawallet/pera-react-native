...
bundle exec fastlane match development \
  --app_identifier "com.algorand.pera.dev,com.algorand.pera.staging,com.algorand.pera" \
  --username "$APPLE_ID" \
...
bundle exec fastlane match adhoc \
  --app_identifier "com.algorand.pera.staging" \
  --username "$APPLE_ID" \
  --team_id "$APPLE_TEAM_ID"
```
...
4. For each configuration (Debug, Staging, Release):
    ...
    - Profile names will be like "match Development com.algorand.pera.dev"
...
2. **Use Readonly Mode**: Always use `readonly: true` in CI to prevent creating new certificates:
    ...
    match(
      type: "appstore",
      readonly: true,
      app_identifier: ["com.algorand.pera"]
    )
    ```
...
- Verify bundle IDs match exactly:
    - `com.algorand.pera.dev`
    - `com.algorand.pera.staging`
    - `com.algorand.pera`
