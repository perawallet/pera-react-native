...
const installProvider = (): void => {
    ...
    window.addEventListener('message', (event: MessageEvent) => {
        ...
        if (!isArc0027Request(request)) {
            // Addressed to us but naming a method we don't implement: answer
            // 4003 rather than dropping it. The drop happened before the
            // pending entry and its timeout below were armed, so the caller's
            // promise never settled at all — see isArc0027NamespacedRequest.
            // Anything outside the namespace is genuinely not ours and is
            // ignored, since the page bus carries everyone's traffic.
            if (isArc0027NamespacedRequest(request)) {
                const method = referenceMethod(request.reference)
                forward({
                    id: globalThis.crypto.randomUUID(),
                    requestId: request.id,
                    // Fixed reference, matching the Chrome router's
                    // untrusted-origin response: an unrecognised method has no
                    // valid reference to echo, and the page correlates by
                    // requestId regardless.
                    reference: 'arc0027:discover:response',
                    error: {
                        code: ARC0027_ERROR_CODES.MethodNotSupportedError,
                        message: `Pera does not support the '${method}' method`,
                    },
                })
            }
            return
