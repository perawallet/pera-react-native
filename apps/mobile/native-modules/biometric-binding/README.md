...
`expo.autolinking.nativeModulesDir` — no config plugin or manual registration.
...
opt-in (PERA-4702 follow-up). Neither `isEnrolledAsync` nor
...
- `Cipher.init` is the probe: it raises `KeyPermanentlyInvalidatedException`
  ...
  BiometricPrompt. `UserNotAuthenticatedException` therefore means intact →
  `'valid'`.
...
Not reachable from the JS suites — the JS side is covered by mocks, so the
