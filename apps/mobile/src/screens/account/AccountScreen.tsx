...
import { DrawerActions, StaticScreenProps, useNavigation } from '@react-navigation/native';
import { AccountWealthHistoryItem, formatDatetime, getAccountDisplayName, useAccountBalances, useAllAccounts, useAppStore, WalletAccount } from '@perawallet/core';
...
const AccountScreen = ({ route }: AccountScreenProps) => {
    ...
    const account = (route.params?.account ?? getSelectedAccount() ?? accounts.at(0))!
    ...
    <MainScreenLayout fullScreen>
      <PeraView style={styles.iconBar}>
        <PeraView style={styles.iconBarSection}>
          <TouchableOpacity style={styles.accountSelection} onPress={toggleAccountSelectorVisible}>
      ...
      >
        ...
        <WealthChart account={account} onSelectionChanged={chartSelectionChanged} />
