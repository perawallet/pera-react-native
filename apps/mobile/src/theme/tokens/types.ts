...
/**
 * The app's own colour tokens. themed.d.ts merges these into rneui's `Colors`,
 * and the light and dark token maps must define every one of them.
 */
export interface AppColors {
    ...
    // Drawer overlay (alpha baked in — see the token files for why it inverts)
    drawerScrim: string
...
/** rneui's built-in colours the token maps override; the rest keep rneui's defaults. */
export type RneuiColorToken =
    | 'primary'
    | 'secondary'
    | 'background'
    | 'black'
...
export type ColorToken = keyof AppColors | RneuiColorToken
