...
import { SignRequest, truncateAlgorandAddress, useAppStore, useTransactionSigner } from "@perawallet/core"
...
const SingleTransactionView = ({ request }: SigningViewProps) => {
    ...
    return <PeraView style={styles.body}>
        <TransactionIcon type="pay" size='large'  />
        <Text h4>Transfer to {truncateAlgorandAddress(tx.receiver)}</Text>
        <CurrencyDisplay currency="ALGO" precision={3} value={Decimal(-5059.44)} showSymbol  h1 h1Style={styles.mainAmount} />
        <CurrencyDisplay currency="USD" precision={3} value={Decimal(-5059.44 * 0.17)} showSymbol  h3 h3Style={styles.secondaryAmount} />
    </PeraView>
}
...
const SigningView = ({ request }: SigningViewProps) => {
    const styles = useStyles()
    const { theme } = useTheme()
    const { removeSignRequest, accounts } = useAppStore()
    const { signTransactionForAddress } = useTransactionSigner()
    const { showToast } = useToast()

    const isMultipleTransactions = request.txs?.length > 1 || (request.txs?.at(0)?.length ?? 0) > 1
    ...
    return <PeraView style={styles.container}>
        <Text h4 h4Style={styles.title}>{isMultipleTransactions ? 'Review Transactions' : 'Review Transaction'}</Text>
        ...
        <PeraView style={styles.footer}>
            <PeraView style={styles.feeContainer}>
                <Text h4 h4Style={styles.feeLabel}>Transaction Fee</Text>
                <CurrencyDisplay currency="ALGO" precision={3} value={txFees} showSymbol  h4 h4Style={styles.feeAmount} />
            </PeraView>
            ...
            <PeraView style={styles.buttonContainer}>
                <PeraButton title="Cancel" variant="tertiary" onPress={rejectRequest} containerStyle={styles.button} />
                <PeraButton title={isMultipleTransactions ? 'Confirm All' : "Confirm"} variant="primary" onPress={signAndSend} containerStyle={styles.button} />
            </PeraView>
        </PeraView>
    </PeraView>
}
