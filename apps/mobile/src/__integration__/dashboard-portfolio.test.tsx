...
describe('Flow: Dashboard portfolio aggregation', () => {
    ...
    it('Given two signing accounts with seeded ALGO + ASA balances and seeded USD prices, when the portfolio hooks resolve, then portfolioUsdValue equals the sum across accounts and per-account values are exposed', async () => {
    ...
    it('Given the accounts store is empty, when the portfolio hooks resolve, then accountBalances is empty and portfolioUsdValue is zero', async () => {
        useAccountsStore.getState().setAccounts([])
        ...
        expect(result.current.portfolio.portfolioUsdValue?.isZero()).toBe(true)
