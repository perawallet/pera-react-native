...
describe('Flow: Asset discovery (search)', () => {
    it('Given the search endpoint returns mixed-tier results, when the hook resolves, then all three verification tiers (verified, unverified, suspicious) round-trip onto the results so the UI can decorate them', async () => {
        ...
        const byId = new Map(result.current.results.map(r => [r.assetId, r]))
    ...
    it('Given a paged search response (next cursor URL present), when the consumer triggers fetchNextPage, then the second page loads and merges with the first', async () => {
    ...
    it('Given the consumer disables the search (enabled: false), when the hook mounts, then no request fires and results stay empty', async () => {
