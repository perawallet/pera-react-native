...
describe('Flow: Banners (regular)', () => {
    ...
    it('Given the server returns three banners of mixed types, when useVisibleBanners resolves, then all three round-trip with type / title / subtitle / CTA mapped to camelCase', async () => {
        server.use(
            mockBanners({
                deviceID: DEVICE_ID,
                response: {
        ...
        await waitFor(() => expect(result.current.banners).toHaveLength(3), {
            timeout: 5000,
        })
    ...
    it('Given a server payload with two regular banners, when the user dismisses one via the store, then useVisibleBanners filters that banner out without re-fetching', async () => {
        server.use(
            mockBanners({
                deviceID: DEVICE_ID,
                response: {
        ...
        await waitFor(() => expect(result.current.banners).toHaveLength(2), {
            timeout: 5000,
        })
    ...
    it('Given a server payload that includes a forced banner alongside regular banners, when useVisibleBanners resolves, then ONLY the forced banner is returned and it bypasses any client-side dismissal', async () => {
        server.use(
            mockBanners({
                deviceID: DEVICE_ID,
                response: {
    ...
    it('Given the server returns banners missing all renderable text fields, when useBannersQuery resolves, then those banners are filtered out (nothing to render)', async () => {
        server.use(
            mockBanners({
                deviceID: DEVICE_ID,
                response: {
...
describe('Flow: Spot banners', () => {
    ...
    it('Given the server returns spot banners, when useSpotBannersQuery resolves, then API fields map to the SpotBanner domain shape (image→imageUrl, button_url_is_external→isUrlExternal)', async () => {
        server.use(
            mockSpotBanners({
                deviceID: DEVICE_ID,
                response: [
    ...
    it('Given a spot banner is visible, when the dismiss mutation runs, then the close endpoint is called and the next GET excludes the banner', async () => {
        const SPOT_ID = '42'
        const KEEP_ID = '43'
    ...
    it('Given the close endpoint returns an error, when the dismiss mutation finishes, then the optimistic removal is rolled back so the user sees the banner again', async () => {
        const SPOT_ID = '50'
        server.use(
            mockSpotBanners({
                deviceID: DEVICE_ID,
