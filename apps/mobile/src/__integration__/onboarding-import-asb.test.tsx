...
describe('Flow: Onboarding → Import from Algorand Secure Backup', () => {
    ...
    it('Given the recovery-key word slots are rendered, then every slot is a sensitive input so the keyboard neither learns the words nor composes them in an IME', async () => {
        ...
        fireEvent.click(screen.getByTestId('asb_import_backup_continue_button'))
    ...
    it('Given a valid backup file picked from disk and the correct recovery key, the single account lands in the wallet', async () => {
        vi.mocked(File.pickFileAsync).mockResolvedValueOnce(
            fakeFileFor(buildSingleAccountAsbBackup()),
        )
        ...
        fireEvent.click(screen.getByTestId('asb_import_key_continue_button'))
        ...
        fireEvent.click(screen.getByTestId('asb_import_select_continue_button'))
    ...
    it('Given a backup with one single and one watch account, both land in the wallet under their correct types', async () => {
        vi.mocked(File.pickFileAsync).mockResolvedValueOnce(
    ...
    it('Given a valid backup file pasted from the clipboard, the import path works without the file picker', async () => {
        ...
        fireEvent.click(screen.getByTestId('asb_import_backup_paste_button'))
        await waitForButtonEnabled('asb_import_backup_continue_button')
        fireEvent.click(screen.getByTestId('asb_import_backup_continue_button'))
    ...
    it('Given the wrong 12-word recovery key, decryption fails, an error toast is raised, and no account is imported', async () => {
        vi.mocked(File.pickFileAsync).mockResolvedValueOnce(
            fakeFileFor(buildSingleAccountAsbBackup()),
        )
    ...
    it('Given the backup contains an account that is already in the wallet, the recovery succeeds and the duplicate is counted as skipped, not failed', async () => {
    ...
    it('Given the user successfully imports an account, decrypted private keys are zeroed in place and the flow store is empty by the time the result screen renders', async () => {
        vi.mocked(File.pickFileAsync).mockResolvedValueOnce(
            fakeFileFor(buildSingleAccountAsbBackup()),
        )
    ...
    it('Given the user navigates back into the backup screen after the flow has wiped the store, the displayed file is cleared and Continue is disabled', async () => {
