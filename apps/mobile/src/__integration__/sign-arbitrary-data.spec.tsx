...
describe('Flow: arbitrary-data (algo_signData) signing review', () => {
    ...
    it('signs a single arbitrary-data message and delivers the signature to the callback', async () => {
    ...
    it('renders a binary payload as a hex dump with the unreadable-data warning', async () => {
    ...
    it('rejects an arbitrary-data request when the user cancels', async () => {
        const { request, approve, reject } = buildArbitraryDataSignRequest({
    ...
    it('blocks a quantum signer with a terminal notice instead of the confirm control', async () => {
        ...
        expect(screen.queryByTestId('arbitrary-data-confirm-slide')).toBeNull()
