...
describe('Flow: Cloud backup → Restore', () => {
    ...
    it('Given a valid backup passphrase and encryption key, when the user restores, then the encrypted Algo25 account is decrypted and imported into the wallet', async () => {
    ...
    it('Given an HD backup (hdWallet items + an hdSeed secret), when the user restores, then the seed is reconstructed and all HD accounts are imported', async () => {
    ...
    it('Given a backup holding contacts, when the user restores, then the contacts are on the device', async () => {
        const { backupId, encryptionKey, itemKey } = await deriveBackupKeys({
            mnemonic: BACKUP_MNEMONIC,
            salt: BACKUP_SALT,
        })
        ...
        await waitFor(() => {
            const item =
                useBackupSyncStateStore.getState().syncState?.items[contactKey]
            expect(item?.knownVer).toBeGreaterThan(0)
        })
        await expectRestoreLandedOnOverview()
    })

    it('Given an encryption key file saved on the device, when the user imports it and types the passphrase, then the key screen shows the imported key and the backup restores from it', async () => {
        const { backupId, encryptionKey, itemKey } = await deriveBackupKeys({
            mnemonic: BACKUP_MNEMONIC,
            salt: BACKUP_SALT,
        })
