...
describe('Flow: Cold start with no connectivity', () => {
    ...
    it('Given a never-synced install with no connectivity, when a fiat amount is displayed, then the conversion resolves to a placeholder rather than 0', async () => {
    ...
    it('Given a never-synced install holding ALGO and the default USD currency, when the portfolio resolves, then the fiat total is unknown rather than 0', async () => {
    ...
    it('Given a normal online session, when the same surfaces render, then nothing about them changed — real totals, real conversions, and the ordinary empty copy', async () => {
        ...
        expect(
            result.current.currency.usdToPreferred(new Decimal(3))?.toString(),
        ).toBe('2.7')
