...
describe('Edge: Network failure paths', () => {
    ...
    it('Given the user submits a valid send, when algod /v2/transactions returns 503, then the success screen never renders and a failure toast is raised', async () => {
    ...
    it('Given a request that crashes the network layer (transport error, not HTTP error), when the swap quote mutation runs, then the error state surfaces via TanStack Query rather than crashing the consumer', async () => {
        server.use(
            mockSwapProviders({
                response: {
                    results: [
