...
export const RekeyToQuantumSelectTargetScreen = () => {
    const { t } = useLanguage()
    const { targets, handleSelect } = useRekeyToQuantumSelectTargetScreen()

    return (
        <PWScreen
            scroll='never'
            testID='rekey-to-quantum-select-target-screen'
        >
            <ScreenHeader
                title={t('rekey.to_quantum.select.title')}
                description={t('rekey.to_quantum.select.subtitle')}
            />

            <AccountPicker
                accounts={targets}
                onSelect={handleSelect}
                emptyBody={t('rekey.to_quantum.select.empty')}
