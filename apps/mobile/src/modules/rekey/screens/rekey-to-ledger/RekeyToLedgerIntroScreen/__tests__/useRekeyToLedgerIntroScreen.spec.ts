...
vi.mock('@perawallet/wallet-core-config', () => ({
    config: {
        rekeyToLedgerSupportUrl:
            'https://support.perawallet.app/en/article/rekey-ledger/',
    },
}))

describe('useRekeyToLedgerIntroScreen', () => {
    beforeEach(() => {
        vi.clearAllMocks()
    })

    it('handleStartProcess navigates to the SelectTarget screen with the source address', () => {
        const { result } = renderHook(() => useRekeyToLedgerIntroScreen())
        ...
        expect(mockNavigate).toHaveBeenCalledWith('RekeyToLedger', {
            screen: 'RekeyToLedgerSelectTarget',
            params: { sourceAddress: 'SRC_ADDR' },
        })
    })

    it('handleLearnMore opens the ledger support article in the in-app webview', () => {
        const { result } = renderHook(() => useRekeyToLedgerIntroScreen())
        ...
        expect(mockPushWebView).toHaveBeenCalledWith({
            url: 'https://support.perawallet.app/en/article/rekey-ledger/',
        })
    })
})
