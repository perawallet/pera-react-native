...
describe('toConnectionSettingsRow', () => {
    ...
    // `Connection.createdAt`/`lastActiveAt` are always epoch-ms numbers
    // straight off the persisted record. Unlike
    // `WalletConnectConnection.createdAt` (typed `Date`, but persisted via
    // `createJSONStorage` with no reviver, so a rehydrated record carries an
    // ISO *string* at runtime), there is no Date/string rehydration hazard
    // here to guard against — this deliberately does not carry over
    // `toComparableTime` from `connectionsSettingsHelpers.ts`.
