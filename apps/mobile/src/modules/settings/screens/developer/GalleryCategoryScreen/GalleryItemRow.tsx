...
export const GalleryItemRow = ({ item, onPress }: GalleryItemRowProps) => {
    ...
    // Actions (e.g. "Run screenshot tour") aren't reviewable UI, so they get no
    // works/broken status switch — only screens/sheets/dialogs/components do.
    const isReviewable = item.launch.kind !== 'action'
    ...
    return (
        <PWView
            style={[
                ...
                isReviewable &&
                    visited &&
                    (isGood ? styles.rowGood : styles.rowBroken),
            ]}
        >
            <PWTouchableOpacity
