...
describe('usePendingSignaturesBottomSheet', () => {
    beforeEach(() => {
        vi.useFakeTimers()
    ...
    it('returns isVisible=false when no signRequestId is in the store', () => {
        const { result } = renderHook(() => usePendingSignaturesBottomSheet())
    ...
    it('handleClose clears the signRequestId in the store', () => {
        usePendingSignaturesSheetStore.setState({ signRequestId: 'sr-1' })
        mockQueryReturn(buildSignRequest())
