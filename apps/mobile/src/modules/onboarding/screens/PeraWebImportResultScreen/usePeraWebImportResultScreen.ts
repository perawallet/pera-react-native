...
type UsePeraWebImportResultScreenResult = {
...
export const usePeraWebImportResultScreen =
    (): UsePeraWebImportResultScreenResult => {
        const route =
            useRoute<
                RouteProp<OnboardingStackParamList, 'PeraWebImportResult'>
            >()
        const { t } = useLanguage()
        ...
        const { variant, title, body } = useMemo<{
            ...
            if (importedCount > 0 && failedCount === 0) {
                return {
                    variant: 'success',
                    title: t('onboarding.pera_web_import.result.success_title'),
                    body: t('onboarding.pera_web_import.result.success_body', {
                        count: importedCount,
                    }),
                }
            }
            if (importedCount > 0 && failedCount > 0) {
                return {
                    variant: 'warning',
                    title: t('onboarding.pera_web_import.result.partial_title'),
                    body: t('onboarding.pera_web_import.result.partial_body'),
                }
            }
            if (failedCount === 0 && skippedDuplicateCount > 0) {
                return {
                    ...
                    body: t(
                        'onboarding.pera_web_import.result.nothing_new_body',
                    ),
                }
            }
            return {
                variant: 'error',
                title: t('onboarding.pera_web_import.result.failed_title'),
                body: t('onboarding.pera_web_import.result.failed_body'),
            }
        }, [importedCount, skippedDuplicateCount, failedCount, t])

        const handleDone = useCallback(() => {
            // Wipes any decrypted seeds left in the flow store before
