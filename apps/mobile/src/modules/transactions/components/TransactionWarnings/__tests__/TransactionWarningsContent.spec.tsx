/*
 Copyright 2022-2025 Pera Wallet, LDA
 Licensed under the Apache License, Version 2.0 (the "License");
 you may not use this file except in compliance with the License.
 You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
 Unless required by applicable law or agreed to in writing, software
 distributed under the License is distributed on an "AS IS" BASIS,
 WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 See the License for the specific language governing permissions and
 limitations under the License
 */

import React from 'react'
import { render } from '@test-utils/render'
import { describe, it, expect, vi, beforeEach } from 'vitest'
import {
    BottomSheetIdContext,
    useBottomSheetStore,
} from '@modules/bottom-sheet'
import { TransactionWarningsContent } from '../TransactionWarningsContent'
import type { PeraDisplayableTransaction } from '@perawallet/wallet-core-blockchain'
import { useSigningAccounts } from '@perawallet/wallet-core-accounts'

vi.mock('@perawallet/wallet-core-shared', async importOriginal => {
    const actual =
        await importOriginal<typeof import('@perawallet/wallet-core-shared')>()
    return {
        ...actual,
        truncateAlgorandAddress: vi.fn(a => a),
    }
})

vi.mock('@perawallet/wallet-core-accounts', async importOriginal => {
    const actual =
        await importOriginal<
            typeof import('@perawallet/wallet-core-accounts')
        >()
    return {
        ...actual,
        useSigningAccounts: vi.fn(() => []),
    }
})

vi.mock('@perawallet/wallet-core-blockchain', async importOriginal => {
    const actual =
        await importOriginal<
            typeof import('@perawallet/wallet-core-blockchain')
        >()
    return {
        ...actual,
        encodeAlgorandAddress: vi.fn(
            (bytes: Uint8Array) => `ENCODED_${new TextDecoder().decode(bytes)}`,
        ),
    }
})

const mockUserAccount = {
    address: 'USER_ADDR',
    type: 'algo25',
    keyPairId: 'key-1',
}

const renderInSheet = (transaction: PeraDisplayableTransaction) =>
    render(
        <BottomSheetIdContext.Provider value='sheet-1'>
            <TransactionWarningsContent transaction={transaction} />
        </BottomSheetIdContext.Provider>,
    )

describe('TransactionWarningsContent', () => {
    beforeEach(() => {
        useBottomSheetStore.getState().resetState()
        vi.clearAllMocks()
        vi.mocked(useSigningAccounts).mockReturnValue([
            mockUserAccount,
        ] as unknown as ReturnType<typeof useSigningAccounts>)
    })

    it('shows close account warning when sender is a user account', () => {
        const closeTx = {
            sender: 'USER_ADDR',
            paymentTransaction: {
                closeRemainderTo: 'CLOSE_ADDRESS',
            },
            id: 'TX_ID',
        } as unknown as PeraDisplayableTransaction

        const { getByText } = renderInSheet(closeTx)
        expect(getByText('transactions.warning.close_warning')).toBeTruthy()
    })

    it('hides close warning when sender is not a user account', () => {
        const closeTx = {
            sender: 'EXTERNAL_ADDR',
            paymentTransaction: {
                closeRemainderTo: 'CLOSE_ADDRESS',
            },
            id: 'TX_ID',
        } as unknown as PeraDisplayableTransaction

        const { queryByText } = renderInSheet(closeTx)
        expect(queryByText('transactions.warning.close_warning')).toBeNull()
    })

    it('shows rekey warning when sender is a user account', () => {
        const rekeyTx = {
            sender: 'USER_ADDR',
            rekeyTo: {
                publicKey: new Uint8Array([1, 2, 3]),
            },
            id: 'TX_ID',
        } as unknown as PeraDisplayableTransaction

        const { getByText } = renderInSheet(rekeyTx)
        expect(getByText('transactions.warning.rekey_warning')).toBeTruthy()
    })

    it('shows rekey warning even when sender is not a user account', () => {
        const rekeyTx = {
            sender: 'EXTERNAL_ADDR',
            rekeyTo: {
                publicKey: new Uint8Array([1, 2, 3]),
            },
            id: 'TX_ID',
        } as unknown as PeraDisplayableTransaction

        const { getByText } = renderInSheet(rekeyTx)
        expect(getByText('transactions.warning.rekey_warning')).toBeTruthy()
    })
})
