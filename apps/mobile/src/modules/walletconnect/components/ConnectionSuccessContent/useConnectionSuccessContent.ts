...
export type UseConnectionSuccessContentResult = {
    dAppName: string
...
export const useConnectionSuccessContent = (
    request: WalletConnectSessionRequest,
): UseConnectionSuccessContentResult => {
    const { t } = useLanguage()
    const { resolve } = useBottomSheetResult<boolean>()
    const { canReturnToDapp, returnToDapp } = useReturnToDapp()
    const returnContext = useReturnToDappStore(
        state => state.returnContexts[request.clientId],
    )
