...
describe('AssetSortBottomSheet', () => {
    beforeEach(() => {
        ...
        mockUseAssetSortBottomSheet.mockReturnValue({
            sortOptions: [
                { mode: 'alphabeticalAsc', labelKey: 'asset_sort.alphabetical_asc' },
                { mode: 'alphabeticalDesc', labelKey: 'asset_sort.alphabetical_desc' },
    ...
    it('renders all four sort options with correct testIDs', () => {
        render(<AssetSortBottomSheet isVisible={true} onClose={vi.fn()} />)

        expect(screen.getByTestId('asset_sort_option_alphabeticalAsc')).toBeTruthy()
        expect(screen.getByTestId('asset_sort_option_alphabeticalDesc')).toBeTruthy()
