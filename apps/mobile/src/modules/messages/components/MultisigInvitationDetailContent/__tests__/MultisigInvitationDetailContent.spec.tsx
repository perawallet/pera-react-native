...
import { MultisigInvitationDetailContent } from '../MultisigInvitationDetailContent'
...
vi.mock('../useMultisigInvitationDetailContent', () => ({
    useMultisigInvitationDetailContent: () => hookResult,
}))
...
describe('MultisigInvitationDetailContent', () => {
    beforeEach(() => {
    ...
    it('renders one row per participant address', () => {
        const { getByTestId } = renderWithId()
    ...
    it('hides "You included" copy when user is not a participant', () => {
        ...
        const { queryByText } = renderWithId()
