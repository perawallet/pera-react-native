...
const REGISTER_CALL = 'add(LegacyMigrationPackage())'
...
describe('patchMainApplication', () => {
    it('adds the import and registers the package in an existing apply block', () => {
        ...
        // The registration goes inside the existing apply block, not as a new one.
        expect(result).not.toContain('.packages.apply {\n          add')
        expect(result).toMatch(
            /\/\/ Packages that cannot be autolinked yet can be added manually here\.\n\s*add\(LegacyMigrationPackage\(\)\)/,
        )
    })
