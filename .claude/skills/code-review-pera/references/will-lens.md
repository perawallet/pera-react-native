...
- Secrets at build time: "we can inject this at build time using bitrise secrets and locally using env vars — check out `tools/dev/generate-config.sh`." / "one GoogleService-Info.plist and overwrite it from bitrise secrets in the build."
